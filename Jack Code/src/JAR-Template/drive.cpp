...
void Drive::control_exponential(){
  float leftStick = deadband(controller(primary).Axis3.value(), 5) / 100.0;
  float rightStick = deadband(controller(primary).Axis4.value(), 5) / 100.0;
  auto exponential_throttle = [&](float input){return (input) * pow(1.039,abs(input));};
  auto exponential_turn = [&](float input){return (input) * pow(1.025,abs(input));};
  float throttle = exponential_throttle(leftStick) * 100.0;
  //float turn     = exponential_turn(rightStick) * 100.0;
  float turn = rightStick * 100.0;
  DriveL.spin(fwd, to_volt(throttle+turn), volt);
  DriveR.spin(fwd, to_volt(throttle-turn), volt);
}
void Drive::control_cheesy(){
  ...
  if(leftStick > 1.0) {
    leftStick = 1.0;
  } else if(leftStick < -1.0) {
    leftStick = -1.0;
  }
  float rightStick = deadband(controller(primary).Axis1.value(), 5) / 100.0;
  if(rightStick > 1.0) {
    rightStick = 1.0;
  } else if(rightStick < -1.0) {
    rightStick = -1.0;
  }
  auto exponential_throttle = [&](float input){return (input) * pow(1.039,abs(input));};
  float xSpeed = exponential_throttle(leftStick);
  //float xSpeed = leftStick;
  if(xSpeed > 1.0) {
    xSpeed = 1.0;
  } else if(xSpeed < -1.0) {
    xSpeed = -1.0;
  }
  float zRotation = rightStick;
  float leftSpeed = 0.0;
  ...
  if (xSpeed == 0.0) {
    ...
    leftSpeed = xSpeed + fabs(xSpeed) * zRotation;
    rightSpeed = xSpeed - fabs(xSpeed) * zRotation;
  }
  // Normalize wheel speeds
  float maxMagnitude = fmax(fabs(leftSpeed), fabs(rightSpeed));
  if (maxMagnitude > 1.0) {
    leftSpeed = leftSpeed / maxMagnitude;
    rightSpeed = rightSpeed / maxMagnitude;
  }
  DriveL.spin(fwd, to_volt(leftSpeed * 100.0), volt);
  DriveR.spin(fwd, to_volt(rightSpeed * 100.0), volt);
}
